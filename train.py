...
def run(args):
    ...
    # Convert model weights to bfloat16 and move to GPU if available
    ...
    if args.hf_dataset:
        ...
        dataset_all = load_dataset(args.hf_dataset, args.hf_subset)  # load all splits
        ...
        if args.val_split_percent == 0.0 and not args.hf_dataset_validation_split:
            ...
            if args.hf_dataset_validation_split:
                # If there's a dedicated validation split in the dataset, use it
                ...
                # Train will remain the entire train split above
                train_loader = DataLoader(
                    train_dataset,
                    batch_size=args.batch_size,
                    shuffle=True,
                ...
                if update_step % logging_step_interval == 0:
                    ...
                    if args.auto_adjust_log_interval:
                        ...
                        if dt > desired_dt:
                            ...
                            # Logging faster than target => log less often (increase interval)
                            logging_step_interval += 1
    
                epoch_loss += loss.item()
                # Check compute budget again
                if args.compute_budget > 0 and total_flops >= args.compute_budget:
    ...
    # Validation (if we have a separate validation dataset or a loader created)
    avg_val_loss = None
